...
const UserForm = ({ errors, handleBlur, handleChange, touched, values, isLoadingUser, setFieldValue, userDetails, id }) => {
  const [showPassword, setShowPassword] = useState(false);

  const theme = useTheme();
  useEffect(() => {
    if (userDetails) {
      setFieldValue('fname', userDetails?.firstName);
      setFieldValue('lname', userDetails?.lastName);
      // setFieldValue('fname', 'Dipanwita');
      // setFieldValue('lname', 'fname');
      setFieldValue('email', userDetails?.email);
      // setFieldValue('password', userDetails?.password);
      setFieldValue('phone', userDetails?.phone);
      // setFieldValue('role', userDetails?.role?.id);
    }
