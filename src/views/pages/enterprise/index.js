...
import { useEnterpriseStatusUpdate, useGetEnterpriseList } from 'hooks/useEnterpriseHooks';
...
const columns = [
  ...
  // },
  // { id: 'phone', label: 'Primary Cell Phone', minWidth: 100, align: 'center' },
  // {
...
function CollapsibleRow({ row }) {
  ...
  // const userData = JSON.parse(localStorage.getItem('userData'));
  const { mutate: updatedEnterprise, isPending: isLoadingUpdatedEnterprise } = useEnterpriseStatusUpdate();
  return (
    <>
      <TableRow hover role="checkbox" tabIndex={-1}>
        ...
        >
          {row?.contact?.primary?.address ? (
            <>
              {row?.contact?.primary?.address >= 20 ? (
                Box(
                  <>
                    {row?.contact?.primary?.address.slice(0, 20)}...
                    <HtmlTooltip
                      title={row?.contact?.primary?.address}
                      // sx={{
                      //   maxWidth: 500
                      // }}
                    >
        ...
        >
          ...
          {/* {row?.contact?.primary?.cellPhone ? row?.contact?.primary?.cellPhone : 'N.A.'} */}
        </TableCell>
        ...
        <TableCell align="center">
          <HtmlTooltip title={'Edit' + ' ' + row?.organizationName}>
            <IconButton
              onClick={() => {
                navigate(`/dashboard/enterprise/edit-${row?.id}`);
                // navigate(`/dashboard/recycler/edit`);
              }}
            >
              {/* {console.log('oiuytu', row?.id)} */}
...
const EnterpriseList = () => {
  ...
  const { data: allEnterList, isLoading: isAllOrgDataLoading } = useGetEnterpriseList({
    limit: rowsPerPage,
    sortOrder: order,
    sortField: orderBy,
  ...
  // console.log('🚀 ~ EnterpriseList ~ allEnterList:', allEnterList);
  useEffect(() => {
    setTimeout(() => {
      setLoading(false);
    }, 2000);
  ...
  const handleChangePage = (event, newPage) => {
    ...
    setPage(newPage);
    setFirstPageVal(allEnterList?.first);
    setLastPageVal(allEnterList?.last);
  ...
  return (
    <>
      {isLoading ? (
        ...
        <>
          <BreadcrumbsForPage name="All Enterprise Organizations" obj={{ title: 'All Enterprise Organizations', title2: '', url: '' }} />

          <MainCard
            title=""
            isButton={true}
            url="/dashboard/enterprise/add"
          ...
          >
            <TableContainer sx={{ maxHeight: 440 }}>
              {isAllOrgDataLoading ? (
              ...
              ) : allEnterList?.enterprises === undefined || allEnterList?.enterprises.length == 0 ? (
                <StyledGridOverlay>
                ...
                <Table stickyHeader aria-label="sticky table">
                  ...
                  <TableBody>
                    {stableSort(allEnterList?.enterprises, getComparator(order, orderBy))?.map((row, i) => {
            ...
            <TablePagination
              ...
              component="div"
              count={allEnterList?.total ? allEnterList?.total : 0}
