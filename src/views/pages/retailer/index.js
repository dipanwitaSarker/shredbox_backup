...
import { useGetRetailerList, useRetailerStatusUpdate } from 'hooks/useRetailerHooks';
...
const columns = [
  // { id: 'code', label: 'Organization Code', minWidth: 150, align: 'center' },
...
function CollapsibleRow({ row }) {
  ...
  const { mutate: updatedStatus, isPending: isLoadingRetailerUpdateStatus } = useRetailerStatusUpdate();
  return (
    <>
...
const RetailerList = () => {
  ...
  const { data: allRetailerList, isLoading: isAllRetailerDataLoading } = useGetRetailerList({
    limit: rowsPerPage,
    sortOrder: order,
    sortField: orderBy,
  ...
  const handleChangePage = (event, newPage) => {
    ...
    setPage(newPage);
    setFirstPageVal(allRetailerList?.first);
    setLastPageVal(allRetailerList?.last);
  ...
  return (
    <>
      {isLoading ? (
        ...
        <>
          <BreadcrumbsForPage name="All Retailer Organizations" obj={{ title: 'All Retailer Organizations', title2: '', url: '' }} />
          ...
          >
            <TableContainer sx={{ maxHeight: 440 }}>
              {isAllRetailerDataLoading ? (
              ...
              ) : allRetailerList?.retailers === undefined || allRetailerList?.retailers.length == 0 ? (
                <StyledGridOverlay>
                ...
                <Table stickyHeader aria-label="sticky table">
                  ...
                  <TableBody>
                    {stableSort(allRetailerList?.retailers, getComparator(order, orderBy))?.map((row, i) => {
            ...
            <TablePagination
              ...
              component="div"
              count={allRetailerList?.total ? allRetailerList?.total : 0}
